...
"""
This file contains the functions to perform node and edge ablation on a model.
Here, this means running a complete forward pass on a modified model.

In a node ablation setting, the user needs to specify a mask indicating which nodes to keep.
The function intervenes on the forward pass by setting masked nodes to some baseling values (e.g. 0, average, counterfactual, etc.).
Best used with outputs from attribution methods on nodes.

In an edge ablation setting, the user needs to specify a mask indicating which edges to keep.
The function intervenes on the forward pass by considering each node as a separate function, whose inputs are outputs from upstream nodes masked siminarly as in the node ablation setting.
Each node from the same layer is evaluated with a different input and thus needs a different forward pass : this is more costly than node ablation, where all nodes were evaluated in a single forward pass.
Best used with outputs from attribution methods on edges.
"""
...
@torch.jit.script
def __compiled_loop_pot_ali(mask_idx, potentially_alive, up_nz):
...
def edge_ablation(
    ...
    for downstream in topological_order:
        ...
        for upstream in architectural_graph[downstream]:
            ...
            __compiled_loop_pot_ali(mask.indices(), potentially_alive, up_nz)
